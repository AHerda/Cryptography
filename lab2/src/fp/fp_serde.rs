...
pub const fn deser(s: &str, key_name: &str) -> T {
    ...
    while i + key.len() + 3 <= bytes.len() {
        if bytes[i] == b'"' {
            ...
            while j < key.len() {
                if bytes[i + 1 + j] != key[j] { match_key = false; break; }
                j += 1;
            }
...
impl<const P: T> Serialize for Fp<P> {
    ...
    where S: Serializer {
        ...
        match format {
            FieldFormat::Decimal => state.serialize_field("number", &self.0)?,
            FieldFormat::Hex => state.serialize_field("number", &format!("0x{:x}", self.0))?,
            FieldFormat::Base64 => {
                        ...
                        Field::Number => {
                            ...
                            number = Some(match val {
                                Value::String(s) => {
                                    ...
                                    } else if s.len() > 10 { // Simple heuristic for B64
                                        use base64::{Engine as _, engine::general_purpose};
                        ...
                        Field::Modulo => {
                            ...
                            if m != P {
                                return Err(serde::de::Error::custom(format!(
                                    "Modulo mismatch: expected {}, got {}", P, m
                                )));
                            }
                            modulo = Some(m);
                        }
